...
test('IRT decoding', async () => {
  ...
  expect(await IctRequestToken.fromTokenString(tokenString)).toBeInstanceOf(IctRequestToken);
  expect(await (await IctRequestToken.fromTokenString(tokenString)).getTokenString()).toBe(tokenString);
